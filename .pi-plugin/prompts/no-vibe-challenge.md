...
Procedure:
1. Read the current `.no-vibe/active` session (if any) plus the adaptation stack — `~/.no-vibe/PROFILE.md`, `.no-vibe/PROFILE.md`, and every `*.md` under `~/.no-vibe/user/` and `.no-vibe/user/` (sorted by filename) — to calibrate difficulty.
