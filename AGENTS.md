...
- Codex marketplace installs depend on `.agents/plugins/marketplace.json` (snapshot index), not only `.codex-plugin/plugin.json`. Keep the marketplace name and plugin entry (`no-vibe`) in sync with Codex docs/commands (`codex plugin add no-vibe --marketplace no-vibe`).
